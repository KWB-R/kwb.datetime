...
Saving. This is the case for the time zone "Europe/Berlin" that will be used
in the following two examples. In this time zone, the time is given in 
Central European Time (CET) in winter and in Central European Summer Time (CEST)
in summer. 
...
in Berlin, Germany. The clock of the device switches between standard time and
daylight saving time (summer time) and vice versa. In 2017, on 29 of October,
the day when summer time is reverted back to standard time, the recorded
timestamps are:
...
we can add the number of hours ahead of Coordinated Universal Time (UTC), the so
called UTC Offset, to the timestamps, as in the following:
...
Timestamps of this form can be converted to POSIXct objects by setting the 
`format` argument of the `as.POSIXct()` function to `"%F %T%z"`:
...
the additional information of the UTC offset. For this case the package
kwb.datetime provides a function `utc_offset_Berlin_time()` that returns the
UTC offset for times in time zone "Europe/Berlin":

```{r}
# Guess and print the UTC offsets from the timestamps
(offsets <- kwb.datetime::utc_offset_Berlin_time(timestamps))
```

The function requires that the timestamps are sorted in increasing order. 
Otherwise it could not decide between CEST and CET for possibly unambiguous 
timestamps between 02:00 and 03:00 at the day of reverting time from CEST back
to CET.

The offsets can then be used to create timestamps in ISO 8601 format, i.e.
ending in either "+0200" (when referring to CEST) or in "+0100" (when referring
to CET):

```{r}
# Create ISO 8601 timestamps and print them
(iso <- kwb.datetime::append_utc_offset_string(timestamps, offsets))
```

Finally, you can use `iso_to_localtime()` to convert these character strings
to `POSIXct` objects:

```{r}
# Create POSIXct-objects in time zone "Europe/Berlin" and print them
(kwb.datetime:::iso_to_localtime(iso))
```

### What is the solution (one step)?

These three steps are performed in the function
